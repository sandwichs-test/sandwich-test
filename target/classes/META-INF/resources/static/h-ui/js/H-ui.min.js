			...
			var config = $.cookie = function(key, value, options) {
				if (arguments.length > 1 && !$.isFunction(value)) {
					...
					if (typeof options.expires === "number") {
						...
						t
								.setMilliseconds(t.getMilliseconds() + days
										* 86400000)
					}
				...
				var result = key ? undefined : {}, cookies = document.cookie ? document.cookie
						.split("; ")
	...
	if (typeof define === "function" && define.amd) {
		...
		(function($) {
					...
					function cb(e) {
						...
						if (status === "success") {
							...
							if (status) {
								...
								if (s.error) {
									s.error
											.call(s.context, xhr, status,
													errMsg)
								}
								deferred.reject(xhr, "error", errMsg);
								if (g) {
									$.event.trigger("ajaxError", [ xhr, s,
											errMsg ])
								}
							}
						}
			...
			$.fn.ajaxForm = function(options) {
				...
				return this.ajaxFormUnbind().on("submit.form-plugin", options,
						doAjaxSubmit).on("click.form-plugin", options,
...
(function(document, window, index) {
	var responsiveNav = function(el, options) {
		...
		var nav, opts, navToggle, styleElement = document
				.createElement("style"), htmlEl = document.documentElement, hasAnimFinished, isMobile, navOpen;
		var ResponsiveNav = function(el, options) {
			var i;
			this.options = {
				animate : true,
		...
		ResponsiveNav.prototype = {
			...
			resize : function() {
				if (window.getComputedStyle(navToggle, null).getPropertyValue(
						"display") !== "none") {
					isMobile = true;
			...
			_transitions : function() {
				if (opts.animate) {
					var objStyle = nav.style, transition = "max-height "
							+ opts.transition + "ms";
		...
		placeholder = prototype.placeholder = function() {
			...
			$this.filter(
					(isInputSupported ? "textarea" : ":input")
							+ "[placeholder]").not(".placeholder").on({
...
!function($) {
	...
	EmailSug.prototype = {
		...
		create : function(elem, value) {
			...
			fragment = $('<div class="' + this.options.sugClass
					+ '-wrapper" style="' + style + '" />');
			...
			if (this.$suggest) {
				...
				this.$suggest.on("mouseenter click", "."
						+ this.options.sugClass + "-item", function(e) {
					var lis, li;
					li = $(this);
		...
		doSelect : function(keyCode) {
			...
			case 38:
				...
				elems.removeClass("active").eq(this.selectedIndex).addClass(
						"active");
				break;
			case 40:
				this.selectedIndex++;
		...
		this
				.each(function() {
					...
					var str = '<div class="spinner">'
							...
							+ "</div>";
					that.append(str);
					var input = that.find(".input-text"), subtract = that
							.find(".subtract"), add = that.find(".add"), value = parseInt(input
							.val());
					...
					subtract
							.on(
									...
									function() {
										...
										input
												.val(goodsCount <= options.minValue ? options.minValue
														: goodsCount
																- options.dis);
										add.removeClass("disabled");
					...
					add
							.on(
									...
									function() {
										var goodsCount = parseInt(input.val());
										input
												.val(goodsCount >= options.maxValue ? options.maxValue
														: goodsCount
																+ options.dis);
										subtract.removeClass("disabled");
...
!function($) {
	$.fn.togglePassword = function(options) {
		...
		$(s.el).on(
				s.ev,
				function() {
					"password" == $(input).attr("type") ? $(input).attr("type",
							"text") : $(input).attr("type", "password")
				})
	};
...
!(function($) {
	var _iCheck = "iCheck", _iCheckHelper = _iCheck + "-helper", _checkbox = "checkbox", _radio = "radio", _checked = "checked", _unchecked = "un"
			+ _checked, _disabled = "disabled", _determinate = "determinate", _indeterminate = "in"
			+ _determinate, _update = "update", _type = "type", _click = "click", _touch = "touchbegin.i touchend.i", _add = "addClass", _remove = "removeClass", _callback = "trigger", _label = "label", _cursor = "cursor", _mobile = /ipad|iphone|ipod|android|blackberry|windows phone|opera mini|silk/i
			.test(navigator.userAgent);
		...
		if (/^(check|uncheck|toggle|indeterminate|determinate|disable|enable|update|destroy)$/i
			...
			if (typeof options == "object" || !options) {
				var settings = $.extend({
					...
					labelHover : true
				}, options), selector = settings.handle, hoverClass = settings.hoverClass
						...
						|| "hover", area = ("" + settings.increaseArea)
						.replace("%", "") | 0;
				...
				return stack
						.each(function() {
							var self = $(this);
							tidy(self);
							var node = this, id = node.id, offset = -area + "%", size = 100
									+ (area * 2) + "%", layer = {
								position : "absolute",
								top : offset,
								left : offset,
								...
								opacity : 0
									...
									|| "i" + _checkbox
									: settings.radioClass || "i" + _radio, label = $(
									_label + '[for="' + id + '"]').add(
									self.closest(_label)), aria = !!settings.aria, ariaID = _iCheck
									+ "-"
									+ Math.random().toString(36).substr(2, 6), parent = '<div class="'
									+ className
									+ '" '
							...
							parent = self.wrap(parent + "/>")[_callback](
									"ifCreated").parent().append(
									settings.insert);
							helper = $('<ins class="' + _iCheckHelper + '"/>')
									.css(layer).appendTo(parent);
							self.data(_iCheck, {
								o : settings,
								s : self.attr("style")
							}).css(hide);
							...
							helper.on(_click
								...
								var type = event[_type], toggle = /wn|up/
										.test(type) ? activeClass : hoverClass;
								if (!node[_disabled]) {
									if (type == _click) {
	...
	var methods = {
		init : function(settings) {
			return this
					.each(function() {
						...
						self.opt = $.extend(true, {}, $.fn.raty.defaults,
								settings);
						$this.data("settings", self.opt);
						self.opt.number = methods.between(self.opt.number, 0,
								20);
						if (self.opt.path.substring(self.opt.path.length - 1,
								self.opt.path.length) != "/") {
							self.opt.path += "/"
						}
						...
						if (self.opt.score) {
							self.opt.score = methods.between(self.opt.score, 0,
									self.opt.number)
						}
						for (var i = 1; i <= self.opt.number; i++) {
							$(
									...
									{
										src : self.opt.path
												+ ((!self.opt.score || self.opt.score < i) ? self.opt.starOff
														: self.opt.starOn),
										alt : i,
										title : (i <= self.opt.hints.length && self.opt.hints[i - 1] !== null) ? self.opt.hints[i - 1]
												: i
									}).appendTo(self);
							if (self.opt.space) {
						...
						methods.setTarget.call(self, self.opt.score,
								self.opt.targetKeep);
						var space = self.opt.space ? 4 : 0, width = self.opt.width
								|| (self.opt.number * self.opt.size + self.opt.number
										* space);
						if (self.opt.cancel) {
							self.cancel = $("<img />", {
			...
			self.stars
					.bind(
							...
							function(evt) {
								...
								if (self.opt.half) {
									var position = parseFloat((evt.pageX - $(
											this).offset().left)
											/ self.opt.size), diff = (position > 0.5) ? 1
											: 0.5;
					...
					.click(
							function(evt) {
								self.score
										.val((self.opt.half || self.opt.precision) ? $this
												.data("score")
												: this.alt);
								if (self.opt.click) {
									self.opt.click.call(self, self.score.val(),
											evt)
								}
							})
		},
		...
		fill : function(score) {
			...
			for (var i = 1; i <= number; i++) {
				...
				if (self.opt.iconRange && self.opt.iconRange.length > count) {
					...
					if (self.opt.single) {
						icon = (i == score) ? (star.on || self.opt.starOn)
								: (star.off || self.opt.starOff)
					} else {
						icon = (i <= score) ? (star.on || self.opt.starOn)
								: (star.off || self.opt.starOff)
					}
		...
		fixHint : function() {
			...
			if (!isNaN(score) && score > 0) {
				hint = (score <= this.opt.hints.length && this.opt.hints[score - 1] !== null) ? this.opt.hints[score - 1]
						: score
			}
			$this.data("readonly", true).css("cursor", "default").attr("title",
					hint);
		...
		set : function(settings) {
			this
					.each(function() {
						var $this = $(this), actual = $this.data("settings"), clone = $this
			...
			if (this.opt.target) {
				...
				if (!isKeep || score === undefined) {
					...
					if (this.opt.targetType == "hint") {
						score = (score === null && this.opt.cancel) ? this.opt.cancelHint
								: this.opt.hints[Math.ceil(score - 1)]
					} else {
						score = this.opt.precision ? parseFloat(score).toFixed(
								1) : parseInt(score, 10)
					}
				}
		...
		unfixHint : function() {
			for (var i = 0; i < this.opt.number; i++) {
				this.stars
						...
						.attr(
								"title",
								(i < this.opt.hints.length && this.opt.hints[i] !== null) ? this.opt.hints[i]
										: i)
			}
...
!(function($) {
	$.fn.onePageNav = function(options) {
		...
		onePageNav.getPositions = function($this, o) {
			...
			$nav.each(function() {
				var linkHref = $(this).attr("href"), divPos = $(linkHref)
						.offset(), topPos = divPos.top;
				onePageNav.sections[linkHref.substr(1)] = Math.round(topPos)
						- o.scrollOffset
			})
		};
		onePageNav.getSection = function(windowPos, o) {
			var returnValue = "", windowHeight = Math.round($(window).height()
					* o.scrollThreshold);
		...
		onePageNav.scrollChange = function($this, o) {
			...
			var windowTop = $(window).scrollTop(), position = onePageNav
					.getSection(windowTop, o);
			if (position !== "") {
				onePageNav.adjustNav($this, $this.find(
						"a[href=#" + position + "]").parent(), o.currentClass)
			}
		};
		...
		return this.each(function() {
			var $this = $(this), o = $.meta ? $.extend({}, opts, $this.data())
					: opts;
...
(function($) {
	...
	ColorPicker.prototype.init = function() {
		...
		this.$picker.find(".cp-title").toggle(options.title !== undefined)
				.text(options.title);
		this.$menu = this.$picker.find(".dropdown-menu").toggleClass(
				"pull-right", options.pullMenuRight);
	...
	ColorPicker.prototype.addColor = function(color) {
		var hex = color.toCssStr(), options = this.options;
		if (!this.colors[hex]) {
	...
	ColorPicker.prototype.updateColors = function(colors) {
		var $picker = this.$picker, $menu = this.$menu.empty(), options = this.options, colors = colors
				|| this.colors, that = this;
		var bestLineCount = 0;
		...
		if (options.optional) {
			var $li = $('<li><a class="cp-tile empty" href="###"></a></li>')
					.css({
						width : options.tileSize,
						height : options.tileSize
					});
		...
		$menu.css("width", Math.min(bestLineCount, options.lineCount)
				* options.tileSize + 6)
	};
	ColorPicker.prototype.setValue = function(color, notSetInput) {
		...
		if (color) {
			...
			this.$menu.find(".cp-tile[data-color=" + hex + "]").addClass(
					"active");
			this.$.tooltip("hide");
	...
	ColorPicker.prototype.getOptions = function(options) {
		var thisOptions = $.extend({}, ColorPicker.DEFAULTS, this.$.data(),
				options);
		if (typeof thisOptions.colors === "string") {
			thisOptions.colors = thisOptions.colors.split(",")
		}
...
!function($) {
	$.fn.Huitab = function(options) {
		...
		this.each(function() {
			...
			that.find(options.tabBar).eq(options.index).addClass(
					options.className);
					...
					function() {
						if ($(this).next().is(":visible")) {
							if (options.type == 2) {
								...
								$(this).next().slideUp(options.speed).end()
							...
							if (options.type == 3) {
								$(this).next().slideDown(options.speed).end()
										.addClass(options.className);
								if ($(this).find("b")) {
									$(this).find("b").html("-")
								}
							} else {
								that.find(options.mainCell).slideUp(
										options.speed);
								that.find(options.titCell).removeClass(
										options.className);
...
!function($) {
	$.fn.Huitags = function(options) {
		...
		this
				.each(function() {
					var that = $(this);
					var str = '<div class="Huitags-wraper">'
					...
					if (options.tagsDefault) {
						...
						for (var i = 0; i < tagsDefaultLength; i++) {
							has.append("<span>" + options.tagsDefault[i]
									+ "</span>")
						}
						has
								.find("span")
								.on(
										...
										function() {
											...
											taghasV = taghasV.replace(
													/(^\s*)|(\s*$)/g, "");
											editor
													.append('<span class="Huitags-token">'
															+ taghasV
															+ "</span>");
											gettagval(this);
											$(this).remove()
					...
					input.on("keydown", function(e) {
						...
						if (evt.keyCode == keyCodes.Enter
								|| evt.keyCode == keyCodes.Enter2
								|| evt.keyCode == keyCodes.Spacebar) {
	...
	$.fn.Huitextarealength = function(options) {
		...
		this
				.each(function() {
					...
					var v = that.val();
					var l = v.length;
					var str = '<p class="textarea-numberbar"><em class="textarea-length">'
							+ l + "</em>/" + options.maxlength + "</p>";
										...
										timer = setTimeout(
												function() {
													...
													var winW = $(window)
															.width();
													var winW5 = winW / 2;
													var imgT = that.parent()
															.offset().top;
													var imgL = that.parent()
															.offset().left;
													var imgW = that.parent()
															.width();
													var imgH = that.parent()
															.height();
													var ww = (imgL + imgW / 2);
													var tooltipLeft = "auto", tooltipRight = "auto";
													if (ww < winW5) {
														tooltipLeft = (imgW + imgL)
																+ "px"
													} else {
														tooltipRight = (winW - imgL)
																+ "px"
													}
													that
															.addClass(options.className);
													if (bigImg == "") {
														...
														var tooltip_keleyi_com = '<div id="preview-wraper" style="position: absolute;width:'
																+ options.bigImgWidth
																+ "px;height:auto;top:"
																+ imgT
																+ "px;right:"
	...
	$.Huimodalalert = function(info, speed) {
		...
		if (speed == 0 || typeof (speed) == "undefined") {
			...
			$(document.body).append(
					'<div id="modal-alert" class="modal modal-alert radius">'
							+ '<div class="modal-alert-info">' + info
							+ "</div>" + "</div>");
			$("#modal-alert").fadeIn();
...
!function($) {
	$.Huitotop = function(bottom) {
		...
		var str = '<a href="javascript:void(0)" class="tools-right toTop Hui-iconfont" title="返回顶部" alt="返回顶部" style="display:none;bottom:'
				+ bottom + 'px">&#xe684;</a>';
...
!function($) {
	...
	Modal.prototype.show = function(_relatedTarget) {
		...
		this.backdrop(function() {
			...
			transition ? that.$element.find(".modal-dialog").one(
		...
		$(toggle).each(
				function() {
					...
					if (e && e.type == "click"
						...
						return $parent.trigger(e = $.Event("hide.bs.dropdown",
								relatedTarget))
					}
					if (e.isDefaultPrevented()) {
						return
	...
	Tooltip.prototype.getViewportAdjustedDelta = function(placement, pos,
		...
		if (/right|left/.test(placement)) {
			...
			if (topEdgeOffset < viewportDimensions.top) {
				...
				if (bottomEdgeOffset > viewportDimensions.top
...
!function($) {
	...
	Popover.prototype.setContent = function() {
		...
		$tip.find(".popover-content").children().detach().end()[this.options.html ? (typeof content == "string" ? "html"
				: "append")
				: "text"](content);
									...
									if (typeof options === "string") {
										...
										for (var i = 0, len = this.length; i < len; i++) {
											...
											if (!$
													...
													|| options.charAt(0) === "_") {
												logError("no such method '"
														+ options + "' for "
														+ namespace
														+ " instance");
												continue
											}
											var returnValue = instance[options]
													.apply(instance, args);
					...
					(function($) {
						...
						function createNewSlider(element, options) {
							...
							if (this.sliderElem) {
								...
								var createAndAppendTooltipSubElements = function(
										tooltipElem) {
								...
								var sliderTooltip = document
										.createElement("div");
								sliderTooltip.className = "tooltip tooltip-main";
								createAndAppendTooltipSubElements(sliderTooltip);
								var sliderTooltipMin = document
										.createElement("div");
								sliderTooltipMin.className = "tooltip tooltip-min";
								createAndAppendTooltipSubElements(sliderTooltipMin);
								var sliderTooltipMax = document
										.createElement("div");
								...
								[ "left", "top", "width", "height" ].forEach(
										function(prop) {
											this._removeProperty(
													this.trackSelection, prop)
										}, this);
								[ this.handle1, this.handle2 ].forEach(
										function(handle) {
											this
													._removeProperty(handle,
															"left");
							...
							var availableHandleModifiers = [ "round",
									"triangle", "custom" ];
						...
						Slider.prototype = {
							...
							setValue : function(val, triggerSlideEvent) {
								...
								this.options.value = this
										._validateInputValue(val);
								var applyPrecision = this._applyPrecision
										.bind(this);
								if (this.options.range) {
								...
								this._layout();
								var newValue = this.options.range ? this.options.value
										: this.options.value[0];
							...
							_layout : function() {
								...
								if (this.options.reversed) {
									...
									positionPercentages = [ this.percentage[0],
											this.percentage[1] ]
								}
							...
							_mousedown : function(ev) {
								...
								this.percentage[this.dragged] = this.options.reversed ? 100 - percentage
										: percentage;
								this._layout();
								if (this.touchCapable) {
							...
							_keydown : function(handleIdx, ev) {
								...
								this
										._adjustPercentageForRangeSliders(percentage);
							...
							_adjustPercentageForRangeSliders : function(
									percentage) {
								if (this.options.range) {
									if (this.dragged === 0
							...
							_applyToFixedAndParseFloat : function(num,
									toFixedInput) {
							...
							_css : function(elementRef, styleName, value) {
								if ($) {
									...
									var style = styleName.replace(/^-ms-/,
											"ms-").replace(/-([\da-z])/gi,
											function(all, letter) {
				...
				fill : function() {
					...
					for (var i = 0; i < 60; i += this.minuteStep) {
						...
						if (actual.valueOf() < this.startDate
							...
							if (Math.floor(minutes / this.minuteStep) == Math
					...
					case 39:
						...
						if (viewMode == 4) {
							...
							if (viewMode == 3) {
								...
								newViewDate = this
										.moveMonth(this.viewDate, dir)
							} else {
								if (viewMode == 2) {
									newDate = this.moveDate(this.date, dir);
									newViewDate = this.moveDate(this.viewDate,
											dir)
								} else {
									if (viewMode == 1) {
										newDate = this.moveHour(this.date, dir);
										newViewDate = this.moveHour(
												this.viewDate, dir)
									} else {
										if (viewMode == 0) {
											newDate = this.moveMinute(
													this.date, dir);
											newViewDate = this.moveMinute(
													this.viewDate, dir)
										}
									}
								}
						...
						if (viewMode == 4) {
							...
							if (viewMode == 3) {
								...
								if (viewMode == 2) {
									...
									if (viewMode == 1) {
										if (this.showMeridian) {
											newDate = this.moveHour(this.date,
													dir * 6);
											newViewDate = this.moveHour(
													this.viewDate, dir * 6)
										} else {
			...
			var DPGlobal = {
				...
				formatDate : function(date, format, language, type, timezone) {
					...
					if (type == "standard") {
						...
						if (type == "php") {
							val = {
								y : date.getUTCFullYear().toString().substring(
										2),
								...
								t : DPGlobal.getDaysInMonth(date
										.getUTCFullYear(), date.getUTCMonth()),
								j : date.getUTCDate(),
								l : dates[language].days[date.getUTCDay()],
								D : dates[language].daysShort[date.getUTCDay()],
								w : date.getUTCDay(),
								N : (date.getUTCDay() == 0 ? 7 : date
										.getUTCDay()),
